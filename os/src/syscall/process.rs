...
use crate::{
    ...
    mm::{translated_ref, translated_refmut, translated_str, MapPermission, VirtAddr, translated_byte_buffer},
    task::{
        current_process, current_task, current_user_token, exit_current_and_run_next, pid2process,
        suspend_current_and_run_next, SignalFlags, TaskStatus, add_task,
    }, timer::get_time_us,
};
...
pub fn sys_task_info(ti: *mut TaskInfo) -> isize {
    ...
    let current_process = current_process();
    let pcb = current_process.inner_exclusive_access();
    unsafe {
        (*ptr).syscall_times = pcb.syscall_times;
        (*ptr).status = task_status;
        let us = get_time_us() - pcb.start_running_time;
        let tv = TimeVal {
            sec: us / 1_000_000,
            usec: us % 1_000_000,
        };
...
pub fn sys_mmap(start: usize, len: usize, raw_port: usize) -> isize {
    ...
    match pcb.memory_set.insert_framed_area(
        VirtAddr(start),
        VirtAddr(start + len),
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    new_process.exec(&data.read_all().as_slice(),Vec::new());
